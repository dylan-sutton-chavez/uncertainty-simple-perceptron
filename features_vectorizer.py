from feature_encoders.cyclic_time_encoder import CyclicTimeEncoder
from feature_encoders.time_series_features import TimeSeriesFeatures
...
def features_vectorizer(time_series_config: TimeSeriesConfig, time_config: TimeConfig, normalization_config: NormalizationConfig):
    ...
    """
    time_series_features = TimeSeriesFeatures(time_series_config.raw_vector, time_series_config.periods)
    cyclic_time_encoder = CyclicTimeEncoder(time_config.current_minute, time_config.current_hour, time_config.current_day_of_week, time_config.current_month)
...
if __name__ == '__main__':
    """
    Demonstrates the vectorizer, processing sample Alphabet data to print the final combined feature vector.

    Time complexity → O(n1 * m * c) * (n2)

    python -B features_vectorizer.py
    """
