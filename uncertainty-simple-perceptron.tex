...
\begin{abstract}
This paper addresses the challenge of robust signal classification in resource-constrained embedded sensing environments. Vibration signals from rotating machinery, financial time series, and natural language streams share a common property: high entropy and nonstationarity that challenges conventional classifiers. While deep architectures such as long short-term memory networks and transformers provide high representational capacity, their inference overhead and memory requirements are prohibitive for edge and microcontroller applications. This work develops a computationally efficient alternative: a modified linear perceptron featuring an adaptive selective prediction mechanism. 
...
Modern machine learning models comprise multiple algebraic transformations (i.e., hidden layers) \cite{b5} and exhibit a time complexity greater than $\mathcal{O}(d)$ per prediction, resulting in high computational costs during training, inference, and fine-tuning, particularly in embedded edge applications.
...
The late 20th century experienced a \textit{``machine learning winter''}. Models such as the simple perceptron lacked the geometric capacity to solve problems such as XOR \cite{b6}, leading to the decline of these models, and they were largely used as a foundation for subsequent model generations.
...
The applicability of Gaussian score approximations in linear selective classifiers has been previously noted in the selective classification literature \cite{b8}, where score distributions in high-dimensional dot-product models exhibit near-Gaussian behavior as a consequence of the Central Limit Theorem, supporting the use of standard normal critical values for threshold calibration.
...
The introduction of the region $[-\epsilon, \epsilon]$ transforms the usual decision into a geometric margin. Following the principles established by Cortes and Vapnik, maximizing the distance of data points from the separating hyperplane improves generalization \cite{b9}. This approach is consistent with modern selective classification strategies that prioritize precision over coverage \cite{b8}.
...
Central Limit Theorem (CLT). The projected activation z = w·x is a dot product computed as a sum of d terms.

By the CLT, this sum converges in distribution to a Gaussian as d increases, regardless of the marginal distribution of each individual feature $w_i$. This convergence holds even for heavy-tailed inputs, such as financial returns, provided that d is sufficiently large and the terms are not dominated by a single component.

In the proposed framework, the feature engineering pipeline projects the raw signal into a high-dimensional space ($d = 14$ for the NASA IMS task, $d = 1080$ for SemEval, and $d > 20$ for financial datasets). At these dimensionalities, the CLT approximation is well supported, justifying the use of  $Z_{(1 - \frac{E}{2})}$ as a Gaussian critical value for margin calibration. This is consistent with prior work in selective classification that employs Gaussian approximations for score distributions in high dimensional linear models \cite{b8}.
...
\subsection{Sensitivity Analysis of the Calibration Parameter ($\Delta$)}

To assess the robustness of the uncertainty margin to the calibration parameter $\Delta$, the model was evaluated across four representative values on each dataset. Table I summarizes the coverage accuracy trade-off as a function of $\Delta$.

\begin{table}[H]
    \centering
    \caption{Coverage and Accuracy by $\Delta$ Value}
    \label{tab:coverage-accuracy-delta}
    \begin{tabular}{c c c}
        \hline
        $\Delta$ Value & Coverage ($\%$) & Accuracy ($\%$)\\
        \hline
        $0.5$ & $38$ & $82$\\
        $1.0$ & $27$ & $91$\\
        $1.5$ & $17$ & $97$\\
        ...
        Performance summary across different $\Delta$ values, showing how coverage decreases and accuracy increases as the margin threshold is raised.
        }
    \end{tabular}
\end{table}

Results across datasets Type II–IV follow a consistent monotonic pattern:increasing  reduces coverage while improving precision, confirming that the parameter behaves predictably and does not require exhaustive tuning. A value of  $\Delta = 1.5$ was selected as the default for all reported experiments based on its coverage–accuracy balance in the validation partition.
...
GitHub and other resources omitted for double-blind review

\begin{thebibliography}{00}
\bibitem{b1} J. Kim, M. S. Kim, J. H. Park, and J. Kim, ``Uncertainty-Aware Deep Reinforcement Learning Approach for Computational Molecular Design,'' Ind. Eng. Chem. Res., vol. 64, no. 5, 2025.
...
\bibitem{b3} S. Wang et al., ``A Survey on Uncertainty Quantification Methods for Deep Learning,'' ACM Comput. Surv., vol. 57, no. 12, 2025.
\bibitem{b4} A. M. Legendre, ``Nouvelles méthodes pour la détermination des orbites des comètes,'' 1805.
\bibitem{b5} Ronny H., Michael S., ``A brief introduction to deep learning,'' in Deep Learning for Synthetic Aperture Radar Remote Sensing, M. Schmitt and R. Hänsch, Eds. Oxford, UK: Elsevier, 2025, pp. 25–52.
...
\bibitem{b8} Y. Geifman and R. El-Yaniv, ``Selective classification for deep neural networks,'' in Proc. 31st Annu. Conf. Neural Inf. Process. Syst., Long Beach, CA, USA, Dec. 2017.
