...
class TimeSeriesFeatures:
    def __init__(self, raw_vector: list[float], periods: int):
        """
        Initialize the `TimeSeriesFeatures`, and assemble a vector with a given raw vector of prices and the periods window length.

        Args:
            raw_vector: list[float] → List of closing prices.
            periods: int → Integer representing the length of the time window to use.
        ...
        self.raw_vector_window = raw_vector[-periods:]
...
if __name__ == '__main__':
    """
    Initialize a raw vector and start the vectorizer vectorizing the data in a window of fourteen periods.

    Time complexity → O(n)

    Initialize → python time_series_features.py
    """
    ...
    time_series_features = TimeSeriesFeatures(raw_vector, 14)
    print(time_series_features.processed)