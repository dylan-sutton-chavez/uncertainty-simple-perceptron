...
if __name__ == '__main__':
    """
    Run this code, just if the code is runed directly: create a vector, compute the mean and std object, and calculate the z-score for each floatant.

    Time complexity → O(n)

    Initialize → python z_score.py
    """
    vector: list[float] = [1, 2, 3, 4]

    z_score = ZScore(vector=vector)

    for floatant in vector:
        number_z_score: float = z_score.compute_z_score(floatant)
        print(number_z_score)